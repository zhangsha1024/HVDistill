...
from MinkowskiEngine import MinkowskiAvgPooling
...
class Res16UNetBase(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        self.final_2 =  torch.nn.Sequential(
            ...
            conv(
                out_channels, out_channels, kernel_size=[2,2,8], stride=[2,2,8], bias=True, D=D),
            # self.PLANES[7], out_channels, kernel_size=[2,2,8], stride=[2,2,8], bias=True, D=D),
            get_norm(self.NORM_TYPE, out_channels, D, bn_momentum=bn_momentum),
            
            conv(
            out_channels, out_channels, kernel_size=[2,2,5], stride=[2,2,5], bias=True, D=D),
            get_norm(self.NORM_TYPE, out_channels, D, bn_momentum=bn_momentum),
            
            conv(
            out_channels, out_channels, kernel_size=[1,1,2], stride=[1,1,2], bias=True, D=D),
    ...
    def forward(self, x):
        ...
        out_1 = self.final_1(out)
        
        if self.normalize_feature:
            out_1 = SparseTensor(
                out_1.F / torch.norm(out_1.F, p=2, dim=1, keepdim=True),
                coordinate_map_key=out_1.coordinate_map_key,
                coordinate_manager=out_1.coordinate_manager,
            )

        out_2 = self.final_2(out_1)
        if self.normalize_feature:
            out_2 = SparseTensor(
                out_2.F / torch.norm(out_2.F, p=2, dim=1, keepdim=True),
                coordinate_map_key=out_2.coordinate_map_key,
                coordinate_manager=out_2.coordinate_manager,
            )
