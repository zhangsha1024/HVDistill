...
from .vtransforms_hvd import DepthLSSTransform
...
class DilationFeatureExtractor_hvdistill(nn.Module):
    """
    Dilated ResNet Feature Extractor
    """

    def __init__(self, config, preprocessing=None):
        super(DilationFeatureExtractor_hvd1, self).__init__()
        assert (
        ...
        self.bridge = nn.Sequential(
            nn.Conv2d(config["model_n_out"], config["model_n_out"]*2, 2,stride=2),
            nn.BatchNorm2d(config["model_n_out"]*2),
            nn.ReLU(True),
            nn.Conv2d(config["model_n_out"]*2, config["in_channels"], 3,stride=2,padding=1),
            nn.BatchNorm2d(config["in_channels"]),
        )

        self.decoder_2 = DepthLSSTransform(config["in_channels"],config["model_n_out"],config["image_size"],config["feature_size"],
        config['xbound'],config['ybound'],config['zbound'],config['dbound'],config['downsample'],config['depth_ones'])


        self.preprocessing = preprocessing
        self.normalize_feature = config["normalize_features"]

    def forward(self, x,points,lidar2ego,lidar2camera,lidar2image,camera2ego,camera_intrinsics,lidar_aug_matrix,image_aug_matrix,):
        if self.preprocessing:
            x0 = self.preprocessing(x)
        x0 = self.encoder(x0)
        x_1 = self.decoder(x0)
...
class PPKTFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, preprocessing=None):
        ...
        self.decoder_2 = DepthLSSTransform(config["in_channels"],config["model_n_out"],config["image_size"],config["feature_size"],
        config['xbound'],config['ybound'],config['zbound'],config['dbound'],config['downsample'])


    def forward(self, x,points,lidar2ego,lidar2camera,lidar2image,camera2ego,camera_intrinsics,lidar_aug_matrix,image_aug_matrix,):
        if self.preprocessing:
            x = self.preprocessing(x)
        x_1 = self.decoder(self.encoder(x))
        if self.normalize_feature:
            x_1 = F.normalize(x_1, p=2, dim=1)
