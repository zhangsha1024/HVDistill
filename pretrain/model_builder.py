from model import (
    MinkUNet_Hvdistill,
    VoxelNet,
    DilationFeatureExtractor_hvdistill,
...
def make_model(config):
    ...
    """
    # import importlib
    # plg_lib = importlib.import_module('model.former.transformer_perception')
    # importlib.import_module('model.former.encoder')
    if config["model_points"] == "voxelnet":
        model_points = VoxelNet(4, config["model_n_out"], config)
    else:
        model_points = MinkUNet_Hvdistill(1, config["model_n_out"], config)
    if config["images_encoder"].find("vit_") != -1:
        model_images = DinoVitFeatureExtractor(config, preprocessing=Preprocessing())
    elif config["decoder"] == "dilation_2":
        model_images = DilationFeatureExtractor_hvdistill(config, preprocessing=Preprocessing())
