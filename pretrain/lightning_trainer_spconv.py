...
class LightningPretrainSpconv(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        output_points,indices = self.model_points(batch["voxels"], batch["coordinates"],batch["batch_size"])
        self.model_images.eval()
        self.model_images.decoder.train()
        output_images = self.model_images(batch["input_I"],batch["points"],batch["lidar2egos"],batch["ego2globals"],batch["lidar2cameras"],batch["lidar2images"],batch["camera2egos"],batch["camera_intrinsicss"])
        output_images = output_images[indices[:,0].type(torch.long),:,indices[:,1].type(torch.long),indices[:,2].type(torch.long)]
    ...
    def loss_bev(self,batch,output_points, output_images):
        k = output_points
        q = output_images
        return self.criterion(k, q)
