    ...
    def __init__(self, phase, config, transforms=None, cached_nuscenes=None):
        ...
        if phase != "test":
            if cached_nuscenes is not None:
                ...
                # )
                self.nusc = NuScenes(
                version="v1.0-trainval", dataroot="/mnt/petrelfs/zhangsha/dataset/nuscense_test", verbose=False
            )
        else:
            self.nusc = NuScenes(
                version="v1.0-test", dataroot="/mnt/petrelfs/zhangsha/dataset/nuscense_test", verbose=False
            )
